...
//
    try {
      const items = await props.getAllDeps(props.profile.organization["_id"])
...
<div className="d-flex justify-content-center "><div className=""  style={{fontSize:24, fontWeight:"bolder", marginLeft:10, color:"#666"}}>{props.profile.organization?props.profile.organization.name:""}</div></div>
<Breadcrumbs  current={""}/>
</div>
...
<div className='container d-flex flex-column align-items-center flex-md-row justify-content-between my-2 mx-auto mx-md-2 '>
  ...
  <OutlinedInput
         ...
         {(props.profile.role=="orgAdmin"||props.profile.role=="depAdmin") ? <AddItemModal render={() => setRender(!render)} orgId={orgId}/>:""}
