...
const OneUser = ({ user, render, deleteUser ,currentUser}) => {
    ...
    <div className="w-100 m-2 p-2 d-md-flex border  flex-xs-column justify-content-md-between  rounded ">
      <div className="container d-md-flex flex-xs-column-reverse justify-content-md-between">
        ...
        <div className=" m-2 p-2 d-flex flex-column justify-content-center  ">
          ...
          {user&& user.role!=="superAdmin"?<div><strong>Institut: </strong> {user.organization && user.organization.name}</div>:""}
          <div><strong>Email: </strong> {user.local && user.local.email}</div>
      ...
      <div style ={{marginRight:30}} className="m-2 p-2 d-flex flex-md-column justify-content-center">
      {/* empty space */}
    </div>
     : (
     (user.role!="superAdmin"&&currentUser.role!="superAdmin"&&user.role!="orgAdmin"&&currentUser.role!="orgAdmin")?
...
export default connect(mapStateToProps, { deleteUser })(OneUser);