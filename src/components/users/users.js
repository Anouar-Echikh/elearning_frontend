...
const filterListUsers=(users)=>{
  if(props.profile.role=="superAdmin"){
    let filtredFromProfessors=users.filter((el)=>el.role!="superAdmin"|| el.role!="orgAdmin")
    return( filtredFromProfessors)
    }else{
      let filtredListFromSuperAdmin=users.filter((el)=>el.role!="superAdmin"&&el.role!="professor");
      console.log("profile:",props.profile)
     
      let filtredList=filtredListFromSuperAdmin.filter((el)=>el.organization==props.profile.organization["_id"])
      return(filtredList)
    }
}
...
<div className="loader-view">
  ...
  return (
    ...
    <div className="w-100 d-flex flex-column justify-content-center " >
      <div className="m-2 container"  style={{fontSize:24, fontWeight:"bolder", color:"#666"}}>Administrateurs</div>
