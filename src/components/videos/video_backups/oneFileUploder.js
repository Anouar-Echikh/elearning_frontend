...
import {apiRestoreCustomMongoDB,apiDeleteFileFromServer} from "../../../api/apiBackups"
...
const handleContinueButton =async()=> {
  if(uploadSuccess){
    setLoading(true)
  await apiRestoreCustomMongoDB(fileName)
...
<div className="d-flex flex-column justify-content-center align-items-center">
  <h2 className="m-2" style={{color:"inherit"}}>Restauration...</h2>
  <CircularProgress color="inherit" size={35} sx={{marginTop:10}}/>
  </div> 
 
</Backdrop> */}
      <LoadingButton endIcon={<RestoreIcon/>} variant="outlined" loadingPosition="end" loading={loading} color="primary" className=" m-1" onClick={handleOpen} ><span>Restaurer</span></LoadingButton>
<Dialog
        open={open}
        onClose={ handleClose}
      ...
      >
        <DialogTitle id="alert-dialog-title" color="red">Uploader un fichier Backup (.JSON)</DialogTitle>
        <DialogContent>
     < Dashboard
      uppy={uppy}
      inline={true}
      ...
      locale={
      {
        strings: {
          ...
          poweredBy: 'Elearning-Academy.tn',
    ...
    />
        ...
        <DialogActions>
        <LoadingButton endIcon={<RestoreIcon/>} variant="outlined" loadingPosition="end" loading={loading} color="primary" className=" m-1" onClick={handleContinueButton} ><span>Restaurer</span></LoadingButton>
        
          <Button onClick={handleClose} color="primary" disabled={loading} autoFocus>
            Annuller
          </Button>
        </DialogActions>
      </Dialog>
