...
import {getAllVideosByProf} from '../../redux/actions/videosActions'
...
try{
const Videos=await props.getAllVideosByProf(props.profile._id)
...
<div className="my-2"  style={{fontSize:24, fontWeight:"bolder", marginLeft:10, color:"#666"}}>Pr. {props.profile.name}</div></div>
<Breadcrumbs level1={depTitle} current={"Mes videos"}  />
</div>
...
<div className='container d-flex flex-column align-items-center flex-md-row justify-content-between my-2 mx-auto mx-md-2 '>
  <div style={{fontSize:24, fontWeight:"bolder"}}>Tous mes videos</div>
  ...
  <OutlinedInput
     ...
     <div className='mx-md-3 ' >
                ...
                { 
                 (listVideos.length<1?<center>No items!</center>:listVideos.map((el,index)=>(
...
export default connect (mapStateToProps,{getAllVideosByProf})(ListVideos)