...
class LockComponent extends Component {
  ...
  sendSMS = async () => {
    const { sendCodeBySMS } = this.props;
    //get crypted code from localStorage
    let code = localStorage.getItem("ZuL8_i");
    //decrypt request data
    let decrypted_code = this.decrypt_code(code);
    console.log("decryptedCode:" + decrypted_code);
    ...
    var smsOpts = {
      number: this.props.foundUser.phoneNumber,
      textSMS: ` Votre code sécurité est: ${arrData[0]}`
    };
    const val = await sendCodeBySMS(smsOpts);

    if (val) {
      this.props.history.push(`/getSecurityCodeBySMS`);
      // this.props.history.push({
      //   pathname: "/getSecurityCodeBySMS",
      //   state: {
      //     data: encrypted_req
      //   }
      // });
    } else {
      alert(this.props.error_sms);
    }
  };
                      ...
                      render={({
                        ...
                        <div>
                          <div>
                            ...
                            <label>
                              <strong>
                                {this.props.foundUser.local.email}
                              </strong>
                            </label>
                            <br />
                            Merci de vérifier que vous avez reçu un e-mail avec
                            votre code. Celui-ci est composé de 6 lettres.
                          </div>
                          ...
                          <Row>
                            <Col>
                              ...
                              >
                                Vous n’avez plus accès à votre boite e-mail ?
                              </Button>{" "}
                            </Col>
                          </Row>
                          <Row>
